...
# -*- coding: utf-8 -*-
#
# ../ELE610/py3/clsRWS.py      (KS renamed file when copied to his location)
#
#  The class RWS   communicating with RobotWare through Robot Web Services
#                  minor changes KS compared with original, rwsuis by OCH and MI.
#  rwsuis by OCH and MI as UiS bachelor project May 2020 (see OneDrive)
#
# Example on how to use this class (after file is copied to project, i.e. work catalog)
#   from clsRWS import RWS
# An alternative way is to do it the standard way for Python classes, 
# as prepared by Ole Christian and Markus, and install the file in Python location
#   (py37) C:\..\py3> pip install rwsuis
#   >>> import rwsuis
#   >>> from rwsuis import RWS   # an alternative
...
class RWS:
    ...
    def set_robtarget_rotation_quaternion(self, var, rotation_quaternion):
        ...
        self.set_rapid_variable(var, "[[" + ','.join(str(s) for s in trans) + 
            "],[" + ','.join(str(s) for s in rotation_quaternion) + 
            "],[-1,0,0,0],[9E+9,9E+9,9E+9,9E+9,9E+9,9E+9]]")
